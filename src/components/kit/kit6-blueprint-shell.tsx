...
import { blueprintSections, type BlueprintSection } from '@/components/kit/kit6-blueprint-sections';
...
export function Kit6BlueprintShell({
  sectionId,
  children,
}: {
  sectionId: string;
        ...
        <aside className="hidden w-[224px] shrink-0 border-r border-[#e6eaf2] bg-white px-4 py-6 lg:flex lg:flex-col">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[16px] font-medium tracking-[-0.025em] text-[#101828]">PreMeet</div>
                <div className="mt-0.5 text-[12px] text-[#667085]">AI Meeting Intelligence</div>
              </div>
            </div>
              ...
              <div key={group} className={index === 0 ? '' : 'mt-8'}>
                ...
                <div className="space-y-1">
                  {sections.map((section) => (
                    <SidebarItem
                      key={section.id}
                      href={`/kit6-blueprint/${section.id}`}
                      label={section.title}
