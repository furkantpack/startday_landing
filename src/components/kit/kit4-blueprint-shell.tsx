...
import { blueprintSections, type BlueprintSection } from '@/components/kit/kit4-blueprint-sections';
...
export function Kit4BlueprintShell({
  sectionId,
  children,
}: {
  sectionId: string;
  ...
  const breadcrumbs = [
    ...
    { label: 'ProveKit', href: '/kit4' },
    { label: current.title, href: `/kit4-blueprint/${current.id}` },
  ];
              ...
              <div key={group} className={index === 0 ? '' : 'mt-8'}>
                ...
                <div className="space-y-1">
                  {sections.map((section) => (
                    <SidebarItem
                      key={section.id}
                      href={`/kit4-blueprint/${section.id}`}
                      label={section.title}
