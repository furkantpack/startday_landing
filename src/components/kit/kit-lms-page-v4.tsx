...
const lessonGroups = [
  {
    ...
    items: [
      ...
      { label: 'Opportunity Brief', icon: RiSparkling2Line, href: '#section-01' },
      { label: 'Product Spec', icon: RiFileList3Line, href: '#section-02' },
...
const pageToc = [
  ...
  { id: 'section-01', label: 'Opportunity Brief' },
  { id: 'section-02', label: 'Product Spec' },
...
const documentSections = [
  {
    ...
    summary:
      'Frame the startup opportunity with a simple market-entry lens: user pain, timing, founder advantage, and why the niche is worth validating now.',
    bullets: [
      'Define the problem in one sentence and anchor it to a visible workflow bottleneck.',
      'Clarify who feels the pain first: founder-led teams, operators, or niche service businesses.',
      'List the strongest market signals that suggest urgency, willingness to pay, or operational friction.',
    ],
    metric: '3 core signals',
    support: 'Pain, timing, founder fit',
  },
  {
    id: 'section-02',
    eyebrow: 'Section 02',
    title: 'Product Spec',
    summary:
      'Translate the opportunity into a scoped product blueprint with offer framing, asset requirements, launch sequence, and measurable outcomes.',
    bullets: [
      'Outline the MVP promise: what the kit helps the founder ship, test, or learn in the first 7 days.',
      'Separate must-have deliverables from optional polish so the launch stays lean and fast.',
      'Map every asset to a concrete job: landing page, prompt set, email draft, metric sheet, and checklist.',
    ],
    metric: '5 launch assets',
    support: 'Scope, outputs, sequencing',
  },
];
    ...
    <aside className="hidden w-[224px] shrink-0 border-r border-[#e6eaf2] bg-white px-4 py-6 lg:flex lg:flex-col">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[16px] font-medium tracking-[-0.04em] text-[#101828]">Branding Kit</div>
          ...
          <div className="border-b border-[#efebe5] bg-white px-5 py-5 lg:px-10">
            <div className="flex flex-col gap-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[16px] font-medium tracking-[-0.04em] text-[#101828]">ProveKit Blueprint</div>
                  <div className="mt-0.5 text-[12px] text-[#667085]">Opportunity Brief · Product Spec</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="min-w-0 px-5 py-8 lg:px-10">
              <div className="max-w-[920px]">
                ...
                <p className="mt-4 text-[18px] leading-8 text-[#6d665d]">
                  Turn the ProveKit document into a usable startup kit structure with clear sections,
                  founder-ready assets, and a validation-first launch path.
                </p>
                ...
                <div className="mt-14 space-y-6">
                  {documentSections.map((section, index) => (
                    <section
                      key={section.id}
                      id={section.id}
                      className="scroll-mt-24 rounded-[28px] border border-[#ece6dd] bg-white p-6 shadow-[0_12px_28px_rgba(15,23,42,0.04)]"
                    >
                      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                        <div className="max-w-[650px]">
                          <div className="text-[14px] font-medium text-[#8f867b]">{section.eyebrow}</div>
                          <h2 className="mt-2 text-[30px] font-semibold tracking-[-0.05em] text-[#171717]">
                            {section.title}
                          </h2>
                          <p className="mt-3 text-[16px] leading-7 text-[#6d665d]">{section.summary}</p>
                        </div>
                        <div className="min-w-[220px] rounded-[22px] border border-[#ece6dd] bg-[#fbf9f5] p-5">
                          <div className="text-[13px] font-semibold uppercase tracking-[0.12em] text-[#9d9489]">
                            Blueprint focus
                          </div>
                          <div className="mt-3 text-[32px] font-semibold tracking-[-0.06em] text-[#171717]">
                            {section.metric}
                          </div>
                          <div className="mt-2 text-[14px] text-[#7d756b]">{section.support}</div>
                        </div>
                      </div>
                      ...
                      <div className="mt-6 grid gap-4 md:grid-cols-3">
                        {section.bullets.map((bullet, bulletIndex) => (
                          <div
                            key={bullet}
                            className={cn(
                              'rounded-[22px] p-5 shadow-[0_8px_20px_rgba(15,23,42,0.04)]',
                              bulletIndex === 0
                                ? 'bg-[#fff4ea]'
                                : bulletIndex === 1
                                  ? 'bg-[#eef8ff]'
                          ...
                          >
                            <div className="text-[13px] font-semibold uppercase tracking-[0.12em] text-[#8f867b]">
                              Key note {bulletIndex + 1}
                            </div>
                            <p className="mt-3 text-[15px] leading-7 text-[#171717]">{bullet}</p>
                          </div>
                        ))}
                      </div>
                    </section>
