...
export function LoginCard() {
  ...
  return (
    <div className="flex w-full max-w-[400px] flex-col gap-5 rounded-[28px] border border-[var(--stroke-soft-200)] bg-[var(--bg-white-0)] p-6 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <Input.Root>
            <Input.Wrapper>
              <Input.Input id="email" type="email" placeholder="hello@alignui.com" />
            </Input.Wrapper>
          </Input.Root>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="text-[12px] font-medium text-[var(--text-sub-600)]">
              Clear
            </button>
          </div>
          <Input.Root>
            <Input.Wrapper>
              ...
              <button type="button" onClick={() => setShowPassword((s) => !s)}>
                {showPassword ? (
                  <RiEyeOffLine className="size-5 text-[var(--text-soft-400)]" />
                ) : (
                  <RiEyeLine className="size-5 text-[var(--text-soft-400)]" />
                )}
              </button>
            </Input.Wrapper>
          </Input.Root>
      ...
      <div className="flex items-baseline justify-center gap-1 text-[15px] text-[var(--text-sub-600)]">
        Already have an account?
        <LinkButton.Root href="/login" variant="gray" className="text-[var(--text-strong-950)]">
          Login
        </LinkButton.Root>
      </div>
    </div>
